...
const API_CONFIG = {
  ...
  ENDPOINTS: {
    PI_DETAILS: "/formData/all-pi-details",
...
const EDIT_FIELDS = [
  ...
  { name: "detailRate", label: "Rate", type: "select", options: RATE_OPTIONS },
  { name: "detailExchangeRate", label: "Exchange Rate", type: "number" },
  { name: "detailCurrentRate", label: "Rate Per Piece", type: "number" },
  { name: "advanceAmount", label: "Advance Amount", type: "number" },
  { name: "runningSerialNumber", label: "Serial Number", type: "text" },
];
...
const initForm = (fields, record) => {
  ...
  fields.forEach((f) => {
    ...
    if (f.type === "date") form[f.name] = toDateInput(val);
...
const EmptyState = () => (
  <div className="bg-white rounded-xl border border-orange-200 p-12 text-center">
    ...
    <p className="text-gray-600">
      There are no PI details to display at the moment.
    </p>
  </div>
);
...
const PiDetails = () => {
  ...
  const fetchPiDetails = useCallback(async () => {
    try {
      ...
      const response = await makeRequest(
        `${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.PI_DETAILS}?${params}`
      );

      setPiDetails(response.data || []);
      setPagination(response.pagination || DEFAULT_PAGINATION);
    } catch (err) {
      console.error("Failed to fetch PI details:", err);
    }
  ...
  }, [editingRecord, editForm, fetchPiDetails]);
  ...
  const handleExportCSV = useCallback(() => {
    const csvData = piDetails.map((detail, index) => ({
      ...
      Model: detail.model || "-",
      Rate: detail.detailRate || "-",
      "Exchange Rate": formatCurrency(detail.detailExchangeRate),
      "Current Rate": formatCurrency(detail.detailCurrentRate),
      "Advance Amount": formatCurrency(detail.advanceAmount),
      "Serial Number": detail.runningSerialNumber || "-",
      Attachment: detail.attachment || "No attachment",
    }));
    ...
    link.setAttribute(
      "download",
      `pi-details-${selectedYear || "all-years"}-${
        selectedMonth || "all-months"
      }-${new Date().toISOString().slice(0, 10)}.csv`
    );
    link.click();
    URL.revokeObjectURL(url);
  }, [piDetails, currentPage, itemsPerPage, selectedYear, selectedMonth]);
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={piDetails.length === 0 || loading}
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {piDetails.map((detail, index) => (
                      <tr
                        key={detail._id || detail.piNumber || index}
                      ...
                      >
                        ...
                        <TableCell>
                          <span title={detail.detailVendor}>
                        ...
                        <TableCell>
                          <AttachmentLink url={detail.attachment} />
                        </TableCell>
                        <TableCell>
                          <RowActions
